...
async def refreshTokens(request: Request, response: Response):
    ...
    try:
        ...
        refresh(cred)
        response.set_cookie(
            key=REFRESH_TOKEN, value=cred.refresh_token, httponly=True, secure=True)
        response.set_cookie(key=ACCESS_TOKEN, value=cred.token,
...
async def getTokens(request: Request, response: Response):
    ...
    response.set_cookie(
        key=REFRESH_TOKEN, value=targetFlow.result.refresh_token, httponly=True, secure=True)
    response.set_cookie(
        key=ACCESS_TOKEN, value=targetFlow.result.access_token, httponly=True, secure=True)
...
def issueAuthFlow(response: Response, request: Request):
    ...
    response.set_cookie(key=AUTH_FLOW_STATE,
